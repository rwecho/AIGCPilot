...
export function AppSidebar({ initialCategories = [] }: AppSidebarProps) {
  ...
  }, [categories.length, initialCategories.length, setCategories]);
            ...
            >
              <Link href={`/${locale}`} className="flex items-center gap-3">
                <div className="relative bg-primary p-1.5 rounded-lg text-primary-foreground shrink-0 shadow-lg flex items-center justify-center overflow-hidden group/logo">
                  ...
                  {/* 扫光层 */}
                  <div className="absolute inset-0 z-20 w-1/2 h-full bg-linear-to-r from-transparent via-white/30 to-transparent -skew-x-12 animate-shine pointer-events-none" />
                </div>
                {!isCollapsed && (
                  <div className="flex flex-col overflow-hidden">
                    <span className="font-black text-lg tracking-tighter bg-linear-to-r from-primary to-primary/60 bg-clip-text text-transparent leading-none">
                      AIGCPilot
                    </span>
      ...
      <SidebarFooter className="border-t bg-muted/20 p-2">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              asChild
              tooltip={locale === "zh" ? "提交 AIGC" : "Submit AIGC"}
            >
          ...
          {user ? (
            <SidebarMenuItem>
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <SidebarMenuButton
                    tooltip={
                      locale === "zh" ? "账号设置 / 退出" : "Settings / Logout"
                    }
                  ...
                  >
                    ...
                    <span className="flex flex-1 justify-between items-center font-medium overflow-hidden">
                      ...
                      <span className="text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded uppercase font-bold tracking-tighter shrink-0 ml-2">
                        Admin
                      </span>
                    </span>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      {locale === "zh" ? "确认退出登录？" : "Confirm Logout"}
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      {locale === "zh"
            ...
            <SidebarMenuItem>
              <SidebarMenuButton
                asChild
                tooltip={locale === "zh" ? "登录管理" : "Admin Login"}
              >
